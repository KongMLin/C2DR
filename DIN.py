...
class DIN(BaseModel):
    def __init__(self, 
                 ...
                 din_target_field=[("adv_id")],  # 这两个地方指定了需要计算注意力的列应该是
                 din_sequence_field=[("ad_click_list_v001")],
                 # 这个地方需要和target同等长度，并且类型都为list
    ...
    def forward(self, inputs):
        ...
        for idx, (target_field, sequence_field) in enumerate(zip(self.din_target_field, 
            ...
            target_emb = target_emb.reshape(target_emb.size(0),-1,self.embedding_dim)
            sequence_emb = sequence_emb.reshape(sequence_emb.size(0), -1, self.embedding_dim)
            # print(target_emb.shape,sequence_emb.shape)
            ...
            # padding_idx = np.array([None for i in range(len(self.feature_map.feature_specs[seq_field]))])

            # mask = torch.from_numpy(np.ones_like(self.feature_map.feature_specs[seq_field],dtype='float64'))

            pooling_emb = self.attention_layers[idx](target_emb, sequence_emb)
            # print(pooling_emb.shape)
...
class DIN_Attention(nn.Module):
    ...
    def forward(self, target_item, history_sequence, mask=None):
        ...
        # print(attention_weight.unsqueeze(-1).shape, history_sequence.shape)
