...
class CSN(BaseModel):
    def __init__(self,
                 ...
                 learning_rate=1e-3,
                 **kwargs):
        super(CSN, self).__init__(feature_map_u, **kwargs)
        ...
        self.layers = [CrossConnectionLayer(embedding_dim=embedding_dim, device=kwargs['gpu']) for _ in
                       range(layer_num)]
    ...
    def forward(self, inputs):
        ...
        X_s = self.embedding_layer_s(
            X[:, self.user_feature_length:self.user_feature_length + self.source_feature_length])
        ...
        pred_dict = {'y_pred': pred_t, 's_pred': pred_s, 'y_true': y[:, 0].unsqueeze(-1),
                     's_true': y[:, 1].unsqueeze(-1)}
        return pred_dict
